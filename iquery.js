(function (window) {
    ...
    iquery.fn = iquery.prototype = {
        ...
        length: 0, // 保证iquery对象在任何情况下都是伪数组对象
        // 创建init对象，
        ...
        init: function (selector, context) {
            ...
            // handle: 字符串类型
            if (iquery.isString(selector)) {
                // handle: html字符串 eg: '<p>123</p>'
                if (iquery.isHTML(selector)) {
            ...
            // 单一的dom元素
            if (iquery.isDOM(selector)) {
                // 在当前iquery实例上添加索引为0，值为传入的dom元素
                //   此时iquery实例的长度为1
                this[0] = selector;
                this.length = 1;
                return this;
            ...
            // dom数组
            if (iquery.isArrayLike(selector)) {
                // 将dom数组中每一个dom元素添加到当前iquery实例上，并且长度自动更新
                push.apply(this, selector);
                return this;
            }
        },
        each: function (callback) {
            // this就是iquery对象，即each方法的调用者
            iquery.each(this, callback);
            // 实现链式编程，将当前调用者返回
            return this;
        },
        get: function (index) {
    ...
    // 可扩展方法
    iquery.extend = iquery.fn.extend = function (source, target) {
        var k;
    ...
    iquery.extend({
        ...
        isHTML: function (obj) {
            ...
            var _o = iquery.trim(obj);
            return _o.charAt(0) === '<' && _o.charAt(_o.length - 1) === '>' &&
                _o.length >= 3;
        },
        // 判断是否为iquery对象
        isiquery: function (obj) {
            // 小技巧：如果有selector属性就是iquery对象
        ...
        isArrayLike: function (obj) {
            ...
            //  所以将它们过滤掉
            if (iquery.isWindow(obj) || iquery.isFunction(obj)) return false;
    ...
    iquery.extend({
        ...
        parseHTML: function (html) {
            ...
            div.innerHTML = html;
            iquery.each(div.childNodes, function () {
    ...
    iquery.extend({
        ...
        hasClass: function (dom, className) {
            return (' ' + dom.className + ' ')
                    .indexOf(' ' + iquery.trim(className) + ' ') > -1;
        },
        addClass: function (dom, className) {
            var _className = dom.className; // className = ' ' + className;
            if (!_className) dom.className = className;
            else {
                if (!iquery.hasClass(dom, className))
                    dom.className += ' ' + className;
            }
        },
        removeClass: function (dom, className) {
            dom.className = iquery.trim((' ' + dom.className + ' ')
                .replace(' ' + iquery.trim(className) + ' ', ' '));
        },
        toggleClass: function (dom, className) {
            if (iquery.hasClass(dom, className)) {
                iquery.removeClass(dom, className);
            } else {
                iquery.addClass(dom, className);
            }
        }
    });
    iquery.fn.extend({
            ...
            if (value === undefined) {
                ...
                if (typeof name === 'object') {
                    ...
                    return this.each(function () {
                        // this ->遍历到每一个dom元素
                        iquery.setCss(this, name);
                    });
                } else { // 如果name 是一个字符串的话，表示获取相应的样式值
                    // 默认获取iquery对象上的一个dom元素的样式值
                    // 首先判断当前iquery对象上是否有dom元素，
                    // 如果有，就返回第一个dom元素的样式值
                    // 否则就是没有，那么就返回undefined。
                    return this.length > 0 ?
                        iquery.getCss(this[0], name) :
                        undefined;
                }
            } else { // 如果value有值，那么就是iquery对象上的所有dom元素设置单个样式值
                return this.each(function () {
                    iquery.setCss(this, name, value);
                });
            }
        },
        hasClass: function (className) {
            // 默认还是判断iquery对象上的第一个dom元素是否就有指定的样式类
            return this.length > 0 ?
                iquery.hasClass(this[0], className) :
                false;
        },
        addClass: function (className) {
            // 给iquery对象上所有dom元素都添加指定的样式类
            return this.each(function () {
                // this -> 遍历到的每一个iquery对象上的dom元素
                iquery.addClass(this, className);
            });
        },
        removeClass: function (className) {
            return this.each(function () {
                ...
                if (className === undefined) {
                    ...
                    iquery.removeClass(this, className);
                }
            });
        },
        // 如果当前dom有指定的样式类，就删除
        ...
        toggleClass: function (className) {
            return this.each(function () {
                iquery.toggleClass(this, className);
                // if (iquery.hasClass(this, className)) {
                //     iquery.removeClass(this, className);
                // } else {
                //     iquery.addClass(this, className);
                // }
    ...
    iquery.fn.extend({
        attr: function (name, value) {
            ...
            // 默认返回iquery对象上的第一dom元素的指定属性值。
            if (value === undefined) {
                return this.length > 0 ? iquery.getAttr(this[0], name) :
                    undefined;
            } else { // 如果不为undefined，表示给iquery对象上的所有dom元素设置指定的属性值
                return this.each(function () {
                    iquery.setAttr(this, name, value);
                });
            }
        },
        html: function (html) {
            // 如果html没有传入值的话，就表示获取iquery对象上第一个dom元素的innerHTML属性值
            if (html === undefined) {
                return this.length > 0 ? iquery.getHTML(this[0]) : undefined;
            } else { // 如果被传入值，那么就是给iquery对象上的所有dom元素设置innerHTML属性
                return this.each(function () {
                    iquery.setHTML(this, html);
                });
            }
        },
        val: function (v) {
            if (v === undefined) {
                return this.length > 0 ? iquery.getVal(this[0]) : undefined;
            } else {
                return this.each(function () {
                    iquery.setVal(this, v);
                });
            }
        },
        text: function (txt) {
            if (txt === undefined) {
                return this[0] && iquery.getText(this[0]);
            } else {
                return this.each(function () {
                    iquery.setText(this, txt);
                });
            }
        }
    });
    ...
    iquery.each(('mouseenter mouseleave mousedown mouseup keypress ' +
        'keydown keyup focus blur click dblclick scroll resize')
            .split(' '),
        function (type) {
            iquery.fn[type] = function (callback) {
    ...
    iquery.extend({
        ...
        getTweens: function (time, locations, target, dur, easingName) {
            var k, o = {};
            for (k in target) {
                o[k] = iquery.easing[easingName](null, time, locations[k],
    ...
    iquery.fn.extend({
        animate: function (target, dur, easingName) {
            return this.each(function (dom) {
                var locations = iquery.getLocations(dom, target),
                    distances = iquery.getDistances(locations, target),
                    startTime,
                    timer = dom.timerId;
                startTime = +new Date;
                // 把时间转换成毫秒值，动画开始时间。
                function render() {
                    ...
                    if (time >= dur) { // 动画正常结束出口
                        ...
                        tweens = iquery.getTweens(time, locations, target, dur, easingName);
                        window.requestAnimationFrame(render);
                    }
                    // 设置dom样式
                    iquery.setStyles(dom, locations, tweens);
                }
    ...
    iquery.extend({
        ...
        Ajax: function (options) {
            ...
            // 将默认Ajax设置拷贝到context
            iquery.extend(iquery.AjaxSetting, context);
            // 将用户自定义设置覆盖context
            iquery.extend(options, context);
